...
def check_events(ai_settings, stats, scoreboard, screen, ship, aliens, bullets, play_button):
    """响应按键和鼠标事件"""
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_play_button(ai_settings, screen, stats, scoreboard, ship, aliens, bullets,
                              play_button, mouse_x, mouse_y)


def check_play_button(ai_settings, screen, stats, scoreboard, ship, aliens, bullets,
                      play_button, mouse_x, mouse_y):
    """玩家单击 play 开始新游戏"""
...
def fire_bullet(ai_settings, screen, ship, bullets):
    """如果没有达到上限，就发送一颗子弹"""
    # 创建一颗子弹，并将其添加到编组 bullets 中
    if len(bullets) < ai_settings.bullet_allowed:
        new_bullet = Bullet(ai_settings, screen, ship)
        bullets.add(new_bullet)


def update_screen(ai_settings, screen, stats, scoreboard, ship, bullets, aliens,
                  play_button):
    """"更新屏幕上的图像，并切换到新图像"""
...
def update_bullets(ai_settings, stats, scoreboard, screen, ship, bullets, aliens):
    """更新子弹的位置，并删除已消失的子弹"""
    # 更新子弹的位置
    bullets.update()
